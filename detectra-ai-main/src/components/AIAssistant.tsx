    ...
    >
      ...
      <div className={`max-w-[82%] rounded-2xl px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap ${
        isUser
          ...
          : 'bg-gray-800/70 border border-gray-700/60 text-gray-200 rounded-tl-sm'
      }`}>
        {msg.content}
      </div>
...
export default function AIAssistant({ result }: { result: AnalysisResult }) {
  ...
  return (
    <div className="flex flex-col h-[560px]">
      ...
      {/* ── Header ── */}
      <div className="flex items-center gap-3 px-5 py-3.5 border-b border-gray-800 bg-gray-900/60 flex-shrink-0">
        ...
        {showInfo && (
          ...
          >
            <div className="px-5 py-3 bg-purple-500/8 border-b border-purple-500/20 text-xs text-gray-400 leading-relaxed">
              ...
              Add <code className="px-1 bg-gray-800 rounded text-purple-300">VITE_HF_TOKEN</code> in your <code className="px-1 bg-gray-800 rounded text-purple-300">.env</code> for higher rate limits.
            </div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {messages.length === 0 && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <div className="bg-gray-800/70 border border-gray-700/60 rounded-2xl rounded-tl-sm px-4 py-3 max-w-sm">
            ...
            <div className="ml-11 space-y-1.5">
              ...
              {QUICK_QUESTIONS.map((q, i) => (
                <motion.button
                  ...
                  disabled={loading}
                  className="group flex items-center gap-2 w-full text-left px-3.5 py-2.5 bg-gray-900 hover:bg-gray-800/80 border border-gray-800 hover:border-purple-500/40 rounded-xl text-gray-400 hover:text-gray-200 text-xs transition-all duration-150"
                >
      ...
      {/* ── Input area ── */}
      <div className="px-4 py-3.5 border-t border-gray-800 bg-gray-900/60 flex-shrink-0">
        <div className="flex gap-2 items-end">
          <textarea
            ...
            disabled={loading}
            rows={1}
            className="flex-1 px-4 py-2.5 bg-gray-800 border border-gray-700 rounded-xl text-sm text-white placeholder-gray-600 focus:outline-none focus:border-purple-500/60 transition-colors resize-none leading-relaxed"
