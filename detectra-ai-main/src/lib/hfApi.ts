...
export function buildVideoContext(result: Record<string, unknown>): string {
  ...
  const events = ((result.surveillance_events as any[]) || [])
    .map(e => {
  ...
  const topObjects = ((result.top_objects as any[]) || [])
    .slice(0, 10)
    .map(o => `  ${o.label}: ${o.count}×`)
    .join('\n');

  const audioEvts = ((result.audio_events as any[]) || [])
    .slice(0, 20)
    .map(a => {
  ...
  const fusionAlerts = ((result.fusion_insights as any[]) || [])
    .filter(f => f.alert)
    .map(f => {
  ...
  const sc = (result.severity_counts as any) || {};
  const langs = ((result.detected_languages as any[]) || [])
...
SURVEILLANCE EVENTS (${((result.surveillance_events as any[]) || []).length} total)
${events || '  None detected'}

PERSONS & TRACKING
Unique persons tracked: ${((result.unique_track_ids as any[]) || []).length}
