...
function MetricCard({ icon: Icon, label, value, sub, color = 'text-cyan-400', bg = 'bg-cyan-500/10', border = 'border-cyan-500/20' }:
  { icon: React.FC<any>; label: string; value: string | number; sub?: string; color?: string; bg?: string; border?: string }) {
  return (
    <div className={`bg-gray-800/50 rounded-xl p-4 border ${border} flex items-start gap-3`}>
...
function KeyFindings({ result }: { result: AnalysisResult }) {
  const findings: { icon: React.FC<any>; text: string; color: string }[] = [];
      ...
      <div className="flex gap-2 flex-wrap">
        <div className="relative flex-1 min-w-40">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-2 bg-gray-800 border border-gray-700 rounded-xl text-sm text-white placeholder-gray-600 focus:outline-none focus:border-cyan-500/60 transition-colors"
          />
        </div>
          ...
          {SEV_OPTIONS.map(s => (
            <button
              ...
              className={`px-2.5 py-1.5 rounded-lg text-xs border capitalize transition-colors ${
                sevFilter === s ? SEV_COLORS[s] : 'bg-gray-800 text-gray-500 border-gray-700 hover:text-gray-300'
              }`}
            >
              {s}
            </button>
          ...
          {filtered.map((ev, i) => (
            <div key={i} className="rounded-xl border border-gray-800 overflow-hidden">
              <button
                ...
                className="w-full flex items-center gap-3 p-3 hover:bg-gray-800/40 text-left transition-colors"
              >
              ...
              <AnimatePresence>
                {expanded === i && (
                  ...
                  >
                    <div className="px-4 pb-3 pt-2 bg-gray-950/60 border-t border-gray-800 grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs">
        ...
        {expanded && (
          ...
          >
            <div className="px-5 pb-5">
              {videoError ? (
                <div className="flex flex-col items-center justify-center h-48 bg-gray-950 rounded-xl border border-gray-800">
                  ...
                  <a href={getVideoUrl(jobId)} target="_blank" rel="noopener noreferrer"
                    className="mt-3 px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white text-xs rounded-lg border border-gray-700 transition-colors">
                    Try direct download
                  </a>
                </div>
              ) : (
...
function Tab({ tabKey, label, icon: Icon, active, onClick }: {
  tabKey: TabKey; label: string; icon: React.FC<any>; active: boolean; onClick: () => void;
}) {
  ...
  useEffect(() => {
    ...
    getJobResult(jobId)
      ...
      .catch(async () => {
        ...
        setError('Results not available — the API server may be offline and no cached results were found.');
      })
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex gap-2 flex-wrap">
            <a href={getRagJsonUrl(jobId!)} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white rounded-xl border border-gray-700 text-xs transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <motion.div
            ...
            className="lg:col-span-2 card-dark overflow-hidden"
          >
            {/* Tab bar */}
            <div className="flex border-b border-gray-800 overflow-x-auto px-2 bg-gray-900/50">
              {TABS.map(t => (
                <Tab
                  key={t.key}
                  tabKey={t.key}
                  ...
                  {fusion_insights.length === 0 ? (
                    ...
                    fusion_insights.map((ins, i) => (
                      <div key={i} className={`p-3 rounded-xl border text-sm ${
                        ins.alert ? 'border-orange-500/30 bg-orange-500/5' : 'border-gray-800 bg-gray-950/40'
                      }`}>
                ...
                <div className="space-y-3">
                  ...
                  {cleanSpeech.length === 0 ? (
                    ...
                    <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
                      {cleanSpeech.map((seg, i) => (
                        <div key={i} className="flex gap-3 p-3 bg-gray-950/50 rounded-xl border border-gray-800">
                  ...
                  {full_transcript && (
                    <details className="mt-3">
                      ...
                      <p className="mt-2 text-gray-400 text-xs leading-relaxed bg-gray-950/50 rounded-xl p-3 border border-gray-800 whitespace-pre-wrap max-h-48 overflow-y-auto">
                        {full_transcript}
                      </p>
                    </details>
                  )}
                ...
                <div className="max-h-96 overflow-y-auto pr-1 space-y-1.5">
                  {audio_events.length === 0 ? (
                    ...
                    audio_events.map((ae, i) => (
                      <div key={i} className="flex items-center gap-4 p-3 bg-gray-950/50 rounded-xl border border-gray-800">
