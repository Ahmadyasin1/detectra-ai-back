...
export default function Profile() {
  ...
  const handleSave = async () => {
    ...
    const { error } = await updateProfile({ full_name: fullName, github_username: githubUsername });
  ...
  const handleCancel = () => {
    if (profile) { setFullName(profile.full_name || ''); setGithubUsername(profile.github_username || ''); }
      ...
      <div className="min-h-screen bg-gray-950 pt-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            className="card-dark p-6 sm:p-8 mb-6">

            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 pb-6 border-b border-gray-800">
            ...
            <div className="flex flex-wrap gap-3">
              <span className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-500/10 text-blue-400 border border-blue-500/25 rounded-xl text-xs font-medium">
                <Shield className="w-3.5 h-3.5" />Free Account
              </span>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.06 }}
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              {!isEditing ? (
                <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-gray-800 text-gray-300 border border-gray-700 rounded-xl text-sm hover:bg-gray-700 hover:text-white transition-all">
                ...
                <div className="flex gap-2">
                  ...
                  <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-gray-800 text-gray-400 border border-gray-700 rounded-xl text-sm hover:bg-gray-700 transition-colors">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <div>
                ...
                {isEditing ? (
                  <input type="text" value={fullName} onChange={e => setFullName(e.target.value)}
                    className="w-full px-4 py-2.5 bg-gray-800 border border-gray-700 rounded-xl text-white placeholder-gray-600 focus:outline-none focus:border-cyan-500/60 transition-colors text-sm"
                    placeholder="Your full name" />
                ) : (
                  <div className="px-4 py-2.5 bg-gray-800/50 rounded-xl text-gray-300 text-sm border border-gray-800">
              ...
              <div>
                ...
                <div className="px-4 py-2.5 bg-gray-800/30 rounded-xl text-gray-500 text-sm border border-gray-800 flex items-center justify-between">
                  {displayEmail}
              ...
              <div>
                ...
                <div className="px-4 py-2.5 bg-gray-800/30 rounded-xl text-gray-500 text-sm border border-gray-800">
                  {memberSince}
                </div>
              </div>
            </div>
